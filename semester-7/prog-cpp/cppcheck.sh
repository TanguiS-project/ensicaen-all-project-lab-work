...
function use_original_warning_message()
{
  cat <<EOF
********************************************************************************
             ...
             YOUR ARE RUNNING A COPY OF THE ORIGINAL cppcheck.sh SCRIPT
           THIS COPY MAY BE OUTDATED. YOU SHOULD USE THE ORIGINAL VERSION:
                    /home/public/sebfourey/CPlusPlus/cppcheck.sh
...
real=$(realpath $0)
if [[ -d /home/public/sebfourey/CPlusPlus/ && $real != /home/public/sebfourey/CPlusPlus/cppcheck.sh ]]; then
  cmp -s $real /home/public/sebfourey/CPlusPlus/cppcheck.sh || use_original_warning_message
fi
...
function check_root() {
  ...
  [ $nsubdirs -eq "1" ]; root_exists=$?
  display_test_result $root_exists "arxiv root" "The root should be a single folder"
